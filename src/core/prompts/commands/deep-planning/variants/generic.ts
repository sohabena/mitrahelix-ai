...
First, determine the language(s) used in the codebase, then execute these commands to build your understanding. For MuleSoft projects, identify RAML/OAS, DataWeave, Mule config XML, and project structure. MuleSoft domains include: API design, integration flows, connectors, transformations. You must tailor them to the codebase and ensure the output is not overly verbose. For example, you should exclude dependency folders such as node_modules, venv or php vendor, etc. These are only examples, the exact commands will differ depending on the codebase.
